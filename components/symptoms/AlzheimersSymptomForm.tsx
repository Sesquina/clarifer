...
const MOOD_OPTIONS = ["depression", "anxiety", "irritability", "apathy"] as const;
...
export default function AlzheimersSymptomForm({ patientId, onSuccess }: Props) {
  const [memoryLoss, setMemoryLoss] = useState(5);
  const [wordFindingDifficulty, setWordFindingDifficulty] = useState(5);
  ...
  const overallSeverity = Math.round(
    (memoryLoss + wordFindingDifficulty + confusion + sleepDisruption + caregiverStress) / 5
  );
      ...
      const res = await fetch("/api/symptoms/log", {
        ...
        body: JSON.stringify({
          ...
          responses: {
            ...
            word_finding_difficulty: wordFindingDifficulty,
            confusion,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <ScaleField label="Word-finding difficulty" name="word_finding_difficulty" value={wordFindingDifficulty} onChange={setWordFindingDifficulty} />
      ...
      <CheckboxGroup
        ...
        selected={moodChanges}
        onChange={setMoodChanges}
      />
