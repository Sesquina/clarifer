...
The pipeline has three environments: local, preview (Vercel preview URLs from sprint branches), and production (`main` → clarifer.com). Full spec: `docs/CLARIFER_WORKSPACE.md` → "Development Pipeline".

Hard rules for Claude Code:
1. **Always work on a sprint branch** (`sprint-N-<slug>`). Never commit directly to `main`.
2. **Never push to `main`.** `git push origin sprint-*` only.
3. **Never merge to `main`.** Merging is always Samira's decision.
4. **Never bypass CI** (no `--no-verify`, no force-push, no direct production deploys).
5. **Never run migrations against production automatically.** Log `MIGRATION REQUIRED:` and stop.

Every sprint report (the SPRINT COMPLETE entry) must include:
```
Preview URL: [auto-generated by Vercel on push to sprint branch]
To merge to production: review preview URL, then merge sprint branch to main.
```

If the preview URL is not yet known at write time, say so — Vercel surfaces it on the PR and in the Vercel dashboard once the build finishes.
