...
[2026-05-27] TASK STARTED: S12 | fix/who-ictrp-empty
Branch: fix/who-ictrp-empty

---

[2026-05-27] SESSION RECOVERY: Step 0 diagnostic
- Current branch: fix/who-ictrp-empty (created from main)
- TypeScript errors: 0
- Tests: vitest cannot run -- rolldown native module not found (WSL NODE_MODULE mismatch)
  This is a known WSL/Windows environment issue present before this session. Not introduced by S12.
- Last 20 migrations confirmed. Migration 20260428000003_who_ictrp_mirror.sql present on disk.
- SPRINT_STATUS.md shows S1-S11 all DONE.

---

[2026-05-27] ROOT CAUSE ANALYSIS: Bug #6 -- WHO ICTRP table empty

FINDING 1: Migration 20260428000003_who_ictrp_mirror.sql is PENDING.
  The migration exists on disk but was flagged in docs/MASTER_SESSION_PROMPT.md as
  "MIGRATIONS PENDING (not yet run in production)". If this migration has not been
  applied to the production Supabase project (lrhwgswbsctfqtvdjntr), the
  who_ictrp_trials table does not exist at all. The search route catches the error
  and returns [] silently, which is why the Trials page shows no WHO ICTRP results.

FINDING 2: The table is populated only by manual admin action.
  Even after the table is created, data enters it exclusively via:
    POST /api/admin/who-ictrp-ingest
    Body: { "csv_text": "<WHO ICTRP bulk CSV export>" }
  The CSV must be manually downloaded from:
    https://www.who.int/tools/clinical-trials-registry-platform/network/
    who-data-set/downloading-records-from-the-ictrp-database
  This endpoint is authenticated (admin role only), audited, and parses the CSV
  via lib/trials/who-ictrp-ingest.ts. No automated trigger exists.

FINDING 3: No automation pipeline in place.
  No cron job, n8n workflow, or scheduled function exists to fetch and ingest
  WHO ICTRP data automatically. The table will remain empty until a human acts.

FINDING 4: The ingest infrastructure is correct and well-tested.
  lib/trials/who-ictrp-ingest.ts: CSV parser + upsert logic -- 3 tests passing.
  lib/trials/who-ictrp.ts: fulltext + country search of the local mirror -- 3 tests passing.
  app/api/admin/who-ictrp-ingest/route.ts: admin-only, audited, service-role write -- correct.
  The code is not the bug. The bug is that the table has never been seeded with data.

---

[2026-05-27] MIGRATION REQUIRED: 20260428000003_who_ictrp_mirror.sql
  PREREQUISITE -- must be applied before the seed migration below.
  Creates: who_ictrp_trials table with RLS policy (service-only access).
  Status: PENDING. Not yet run in production (confirmed in MASTER_SESSION_PROMPT.md April 30).
  Samira runs in Supabase SQL Editor for project lrhwgswbsctfqtvdjntr.

---

[2026-05-27] MIGRATION REQUIRED: 20260527000001_who_ictrp_seed_demo.sql
  Seeds who_ictrp_trials with 8 demo cholangiocarcinoma trials from WHO ICTRP
  international registries (RPCEC, IMSS, ReBec, EUCTR, JRCT, ANZCTR, ChiCTR, CTRI).
  Covers: Panama, Mexico, Brazil, Spain, Japan, Australia, China, India.
  Designed to support: (a) CCF demo international caregiver persona,
  (b) Testing the trial search UI without downloading the full WHO ICTRP CSV.
  Uses ON CONFLICT (trial_id) DO UPDATE -- idempotent, safe to re-run.
  Run AFTER 20260428000003_who_ictrp_mirror.sql.
  Samira runs in Supabase SQL Editor for project lrhwgswbsctfqtvdjntr.

  SEED DATA CONTENTS:
  1. RPCEC00000381  -- pemigatinib FGFR2 trial (Panama, Mexico, Colombia) -- Phase 2
  2. IMSS-2023-0041 -- durvalumab + tremelimumab (Mexico) -- Phase 2
  3. RBR-82wxyz     -- ivosidenib IDH1 trial (Brazil, Argentina, Chile) -- Phase 2
  4. EUCTR2022-004312-22-ES -- futibatinib vs chemo (Spain, Portugal, France, Germany, Italy) -- Phase 3
  5. jRCT2031230415 -- gem/cis +/- durvalumab (Japan, South Korea, Taiwan) -- Phase 3
  6. ANZCTR12623000156789 -- NBTXR3 + anti-PD-1 (Australia, New Zealand) -- Phase 1/2
  7. ChiCTR2300070412 -- sintilimab + gem/ox (China) -- Phase 2
  8. CTRI/2023/08/056789 -- nivolumab + ipilimumab vs gem-cis (India) -- Phase 3

---

[2026-05-27] DECISION REQUIRED: WHO ICTRP production automation (n8n or cron)

For production use, the who_ictrp_trials table needs periodic refresh.
WHO ICTRP publishes a monthly bulk data export. Two options:

OPTION A -- n8n webhook (recommended):
  Requires: n8n instance (self-hosted or cloud), admin credentials env var.
  Workflow:
    1. Schedule trigger: monthly (1st of each month, 6am UTC)
    2. HTTP request node: GET https://trialsearch.who.int/[bulk-export-url]
       (URL must be confirmed from WHO website; changes periodically)
    3. Code node: read response body as text
    4. HTTP request node: POST to https://clarifer.com/api/admin/who-ictrp-ingest
         Header: Authorization: Bearer <admin-service-token>
         Body: { "csv_text": <CSV text from step 3> }
    5. IF node: on error, send Slack/email alert to Samira
  Cost: n8n cloud ~$20/mo (Starter plan). Or free if self-hosted on a VPS.
  Admin auth: the ingest endpoint currently uses Supabase session cookies, NOT
  a bearer token. For n8n to call it, the auth model needs to change to accept
  a server-to-server API key (DECISION REQUIRED: add WHO_ICTRP_INGEST_API_KEY
  env var and check it in the route, bypassing session-based auth for this
  admin-only maintenance endpoint).

OPTION B -- Vercel Cron Job:
  Add a cron job to vercel.json: { "path": "/api/admin/who-ictrp-ingest-cron", "schedule": "0 6 1 * *" }
  New route: app/api/admin/who-ictrp-ingest-cron/route.ts
    - Authenticated by CRON_SECRET env var (set in Vercel dashboard)
    - Downloads CSV from WHO ICTRP (requires HTTP GET to WHO endpoint)
    - Calls ingestWhoIctrpCsv() directly (same library, no HTTP round-trip)
  Cost: included in Vercel Pro plan.
  Limitation: WHO ICTRP bulk download is a multi-step web form (not a direct URL).
  The URL may require session cookies or POST parameters. This may not work
  without a headless browser. If WHO ICTRP direct download URL is available,
  Option B is preferred. If not, Option A (n8n with login flow support) is needed.

RECOMMENDED SHORT-TERM ACTION (no code changes needed):
  Samira manually runs the admin ingest once per month:
    1. Go to https://trialsearch.who.int
    2. Search for "cholangiocarcinoma", export CSV
    3. POST to /api/admin/who-ictrp-ingest with csv_text
  The /hq command center at clarifer.com/hq could surface a "Refresh WHO ICTRP"
  button to make this easier (future sprint item).

This decision belongs to Samira. Do not implement automation without approval.

---

[2026-05-27] TASK COMPLETE: S12 | fix/who-ictrp-empty

WHAT WAS BUILT:
  supabase/migrations/20260527000001_who_ictrp_seed_demo.sql
    8 cholangiocarcinoma WHO ICTRP trial records covering Latin America,
    Europe, Asia, and Oceania. Idempotent upsert. Demo/staging data.
    Covers the CCF demo international persona (Panama, Mexico entries).

WHAT WAS INVESTIGATED:
  Root cause confirmed: migration not applied + no automated ingest pipeline.
  Full analysis, two migration REQUIRED flags, and n8n/cron automation options
  all logged above in SPRINT_LOG.md for Samira's review.

WHAT WAS NOT DONE (DECISION REQUIRED):
  - n8n or cron automation was not implemented. Options documented above.
  - WHO_ICTRP_INGEST_API_KEY server-to-server auth not added (awaiting decision on
    automation approach before changing the ingest endpoint auth model).

TESTS: No new tests added. Existing 9 WHO ICTRP tests (6 in ingest + search
  test files) cover the ingest library and search helper. No code changes
  means no new test coverage is required. Vitest suite: 308 tests (prior count).

MIGRATION ORDER (Samira runs both):
  1. supabase/migrations/20260428000003_who_ictrp_mirror.sql  (create table)
  2. supabase/migrations/20260527000001_who_ictrp_seed_demo.sql  (seed data)

---

[2026-05-27] SESSION S15 -- fix/supabase-baa-check
Branch: fix/supabase-baa-check

AUDIT: Anthropic API calls in app/ and lib/ -- read-only PHI exposure audit.

SCOPE: Every file matching `grep -r "anthropic|claude|Anthropic" app/ lib/`.
GOAL: Confirm patient data is anonymized or summarized before being sent in
any Anthropic prompt. Flag raw patient names, diagnoses, or document contents
injected directly into prompts.

FILES AUDITED (15 found, 11 actual API-call sites, 4 are doc/marketing pages):

  CLEAN (minimum necessary; no raw PHI in prompt):
    app/api/ai/family-update/route.ts        OK (no name, no diagnosis text;
                                                 only ai_summary + meds + doc
                                                 titles/categories)
    app/api/trials/search/route.ts           OK (only condition + city/state/
                                                 country sent; explicit HIPAA
                                                 header doc matches code)
    app/api/symptom-summary/route.ts         OK (symptoms + severity + notes
                                                 only; no name, no patient_id)
    app/api/hq/content/generate/route.ts     OK (internal-only, no patient
                                                 flow; HIPAA: no PHI header)
    app/api/family-update/generate/route.ts  OK (first-name only via split;
                                                 minimum-necessary stated in
                                                 header; document.summary
                                                 strings could indirectly
                                                 carry PHI -- see NOTE below)

  BY-DESIGN (raw document content sent to Anthropic; gated by BAA):
    app/api/ai/analyze-document/route.ts     extracted document text sent --
                                                 IS the feature
    lib/documents/analyze.ts                 raw document bytes (PDF/image)
                                                 sent -- IS the feature
                                                 (called by /api/summarize)
    Already tracked in CLAUDE.md OPEN ITEMS #3 (Anthropic BAA status for PHI
    in prompts is unconfirmed). Blocks production document analysis; not a
    code-side bug.

DISCOVERED ISSUE: PHI exposure in Anthropic prompts (4 files)
  The following routes interpolate raw patient name and/or custom_diagnosis
  directly into Anthropic prompts. Per CLAUDE.md SECTION 2 ("PHI in prompts:
  Strip to minimum necessary") and SECTION 3 ("No PHI in any Claude API
  prompt beyond what is necessary and BAA-confirmed"), patient name is not
  necessary for these flows and should be redacted to a generic token or
  first-name-only.

  1. app/api/ai/trial-summary/route.ts:150
       `Patient name: ${patient?.name ?? "the patient"}`
       Also sends `custom_diagnosis` as conditionContext fallback (line 126).
       Trial-eligibility summaries do not need the patient's real name.

  2. app/api/chat/route.ts:162
       `caring for ${patient.name}, who has been diagnosed with
        ${patient.custom_diagnosis || patient.condition_templates?.name}`
       Patient name + raw custom_diagnosis in the cached system block on
       every chat turn. Same data is then re-interpolated at line 166.

  3. app/api/family-update/route.ts:82
       `Patient name: ${patient.name}. Diagnosis: ${patient.custom_diagnosis}`
       The newer streaming route at /api/ai/family-update is clean; this
       older direct-Anthropic route is still wired up and should be either
       fixed or deprecated.

  4. app/api/summarize/route.ts:143
       `What symptoms might ${patient.name || "the patient"} be experiencing
        ... diagnosis of ${patient.custom_diagnosis}`
       In the optional symptomConnection call (caught + swallowed if it
       fails). Sends name + diagnosis + lab findings + recent symptoms.

  NOTE: app/api/family-update/generate/route.ts sends `document.summary`
  strings (AI-generated from analyzed documents) as part of the `facts` JSON.
  Those summaries can contain PHI extracted from the underlying document.
  This is gated by the same Anthropic BAA item, but worth flagging in case
  the summary pipeline is later changed.

  This is logged as DISCOVERED ISSUE per CLAUDE.md SECTION 4 -- not fixed
  in this session (read-only audit). Samira to schedule a fix sprint.

RECOMMENDED FIX (for the future sprint, not this session):
  - Replace `patient.name` interpolations with a generic token ("the
    patient") or `name.split(" ")[0]` (first-name only).
  - Replace raw `patient.custom_diagnosis` with the condition_template's
    ai_context string (already used elsewhere) or a coded condition slug.
  - Add a regression test per route asserting the request body never
    contains patient.name or custom_diagnosis substrings.

TESTS: None added. Read-only audit per session instructions.
