...
[2026-05-27] SPRINT: S16 fix/sentry-errors
Branch: fix/sentry-errors | Status: COMPLETE, pending Samira review

CONTEXT:
  Sentry config audit (sentry.server.config.ts, sentry.edge.config.ts,
  instrumentation.ts, instrumentation-client.ts, app/global-error.tsx,
  next.config.ts withSentryConfig). All present and correctly wired with
  PHI keyword filtering in beforeSend. DSN currently hardcoded in source
  rather than read from process.env.SENTRY_DSN -- see DISCOVERED ISSUE below.

FIXED (top 3 production error sources -- catches that previously swallowed
exceptions without reporting to Sentry):

  1. app/api/chat/route.ts
     - Added Sentry.captureException(error) in the stream-loop catch
       (was console.error only).
     - Added Sentry.captureException(error) in the outer handler catch
       (was a silent catch returning 500 with no logging).
     - Tags: route=api/chat, phase=stream | handler.

  2. app/api/trials/route.ts
     - Added Sentry.captureException(error) in the ClinicalTrials.gov
       fetch catch (was console.error only).
     - Removed `detail: String(error)` from the 500 response body.
       That field leaked internal error text to the client; not a HIPAA
       BLOCKER (no PHI flows through this catch path) but unnecessary
       information disclosure.
     - Tags: route=api/trials, upstream=clinicaltrials.gov.

  3. app/api/documents/upload/route.ts
     - Added Sentry.captureException(error) in the uploadToStorage catch
       (was a silent catch returning a generic 500 with no logging).
     - This is the most critical of the three -- it sits on the PHI
       document upload path, so the previous silent failure made any
       Supabase Storage outage invisible in production.
     - Tags: route=api/documents/upload, phase=storage.

DISCOVERED ISSUE (broader observability gap, out of scope for this fix):
  96 try/catch blocks across app/ and lib/, but before this fix only
  app/global-error.tsx (auto-generated by the Sentry wizard) actually
  reported to Sentry. Many API routes have catches that return a 400/500
  without ever calling Sentry.captureException, including but not limited
  to: app/api/biomarkers/route.ts, app/api/biomarkers/[id]/route.ts,
  app/api/care-team/create/route.ts, app/api/care-team/[id]/route.ts,
  app/api/auth/reset-password/route.ts, app/api/appointments/[id]/route.ts.
  These are JSON.parse-style catches that return 400 -- low signal noise,
  not all need Sentry. But they should be reviewed sprint-by-sprint.
  Do not fix inline. Assign to a future observability sprint.

DISCOVERED ISSUE: Sentry DSN hardcoded in source.
  sentry.server.config.ts, sentry.edge.config.ts, instrumentation-client.ts
  all hardcode dsn: "https://7401b6f7ab878fadcbd51e22c6b281bc@o4511125988900864.ingest.us.sentry.io/4511126003908608".
  CLAUDE.md Section 2 lists SENTRY_DSN as a server-only env var, so the
  hardcoded value should be replaced with process.env.NEXT_PUBLIC_SENTRY_DSN
  (client) and process.env.SENTRY_DSN (server). Not a HIPAA BLOCKER (DSN is
  not a secret in the strict sense), but it makes per-environment routing
  (staging vs production) impossible. Do not fix inline -- the DSN being
  hardcoded is what the Sentry wizard generated; coordinated env-var migration
  belongs to Sprint 18 (Security Audit) or a dedicated observability sprint.

DISCOVERED ISSUE: node_modules was missing the entire @vitest/* package tree
  on session start, which made the initial `npx vitest run` invocation hang
  (npm-exec retry loop) and effectively return no signal. Recovered with
  `npm install` (added 634 packages, changed 18, audited 874). Suggests a
  prior session left node_modules in a half-installed state -- worth checking
  whether the autonomous session runner installs deps before invoking tests.

PERMISSION / COORDINATION NOTE:
  During this session, a parallel autonomous session checked out
  fix/who-ictrp-empty-v2 and then fix/who-ictrp-empty, performed a
  `git reset --hard` at HEAD on the current branch, and committed
  S12 (WHO ICTRP seed) at 3d7d223. That hard reset wiped my unstaged
  Sentry edits the first time I applied them; I re-applied and staged
  them immediately. Two autonomous sessions on the same working tree
  is fragile -- consider serializing the runner queue or using
